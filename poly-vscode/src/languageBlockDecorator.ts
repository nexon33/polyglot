 ...
 */
export class LanguageBlockDecorator {
    ...
    private languageColors: Record<string, { bg: string; gutter: string; icon: string }> = {
    ...
    private createGutterSvg(color: string): vscode.Uri {
    ...
    public updateDecorations(editor: vscode.TextEditor) {
        ...
        const bgRangesByLang: Map<string, vscode.DecorationOptions[]> = new Map();
        const gutterRangesByLang: Map<string, vscode.DecorationOptions[]> = new Map();

        for (const block of blocks) {
            const lang = this.normalizeLanguage(block.language);
            if (!this.decorationTypes.has(lang)) continue;
    ...
    private findLanguageBlocks(text: string): Array<{ language: string; range: vscode.Range }> {
        const blocks: Array<{ language: string; range: vscode.Range }> = [];
